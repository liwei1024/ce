...
OB_PREOP_CALLBACK_STATUS KCE_ProcessPreCallback(PVOID RegistrationContext, POB_PRE_OPERATION_INFORMATION OperationInformation)
{
...
VOID KCE_ProcessPostCallback(PVOID RegistrationContext, POB_POST_OPERATION_INFORMATION OperationInformation)
{

	//dprintf("ProcessPostCallback");
}

OB_PREOP_CALLBACK_STATUS KCE_ThreadPreCallback(PVOID RegistrationContext, POB_PRE_OPERATION_INFORMATION OperationInformation)
{
...
VOID KCE_ThreadPostCallback(PVOID RegistrationContext, POB_POST_OPERATION_INFORMATION OperationInformation)
{
	//dprintf("ThreadPostCallback");
}


NTSTATUS KCE_ProtectTheCurrentProcess()
{
	...
	opReg[0].PreOperation = KCE_ProcessPreCallback;
	opReg[0].PostOperation = KCE_ProcessPostCallback;

	opReg[1].ObjectType = PsThreadType;
	opReg[1].Operations = OB_OPERATION_HANDLE_CREATE | OB_OPERATION_HANDLE_DUPLICATE;
	opReg[1].PreOperation = KCE_ThreadPreCallback;
	opReg[1].PostOperation = KCE_ThreadPostCallback;
...
VOID KCE_UnprotectTheCurrentProcess()
{
